...
- **Replay status**: `--replay-db` and `--compare-db` load, re-simulate, and diff encoded event streams. Production instrumentation exists (`scriptbots-core/src/replay.rs` projects per-agent Action events in canonical `AgentUid` order) and is opt-in through `replay_event_tick_cap`, which defaults to `0` so an ordinary run stays byte-identical; the mock-free terminal→FrankenSQLite→export→replay E2E is `crates/scriptbots-app/tests/replay_e2e.rs`. Nonzero empty-vs-empty runs fail closed instead of claiming verification. `bd-2z0.8.9.8` is closed; `bd-a9h1` tracks a live defect in the cap's semantics (it bounds the cross-tick buffer rather than the tick, so most ticks in an interval can record zero action events).

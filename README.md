...
- Set a fixed seed in config: `rng_seed = <u64>`. At runtime you can apply via REST:
  ...
  { "rng_seed": 42 }
  ```
- The root seed deterministically derives all six domain streams. `RunManifestV3` records that root plus the exact versioned `random_streams` checkpoint; manifests carrying current adapter-attested `WorldDigestV1` bootstrap evidence use `scriptbots.run-manifest.v3.2`. The manifest never collapses the domains back into one ambiguous global stream.

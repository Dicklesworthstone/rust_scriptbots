...
- **Replay status**: `--replay-db` and `--compare-db` load, re-simulate, and diff encoded event streams. Production instrumentation projects per-agent actions in canonical `AgentUid` order through `replay_event_tick_cap` (default `512`); `bd-a9h1` tracks the remaining cross-tick action-cap defect. Typed combat and food-share edges use a separate deterministic per-tick prefix cap, `interaction_event_tick_cap` (default `512`, `0` for aggregates only), and preserve source tick, core ordinal, stable actor/target UIDs, positions, and magnitude in the FrankenSQLite `interactions` table. The selected cap is serialized with run provenance, but window/stride sampling, persisted truncation totals, and 5k write-budget evidence remain open on `bd-2z0.5.9`. The mock-free terminal→FrankenSQLite→export→replay E2E is `crates/scriptbots-app/tests/replay_e2e.rs`, and nonzero empty-vs-empty runs fail closed instead of claiming verification.

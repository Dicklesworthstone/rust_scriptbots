...
- **Hard-bounded SQL is refused:** the pinned `fsqlite =0.1.16` async query methods check a caller context before dispatch, but that context does not accompany the statement executing on the `!Send + !Sync` connection-owner worker. The facade exposes no running-statement interrupt that could guarantee a wall-clock stop across engine work or blocking I/O. `AsyncReadLane::open(path, maximum_duration)` therefore always returns a typed `ReadExecutionBoundUnavailable` before the path is opened or inspected; there is no async SQL escape hatch. Use `AnalyticsSnapshotProvider::snapshot()` for lock-free, non-SQL frontend latest-state access. Offline/reporting callers that can wait may use the synchronous, create-free, read-only `StorageReader`; its row limits bound results and allocation, not scan, aggregation, sort, open, close, or wall-clock work.

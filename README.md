...
- **Scheduled patches are currently disconnected**: scenario `interventions` are retained in launch identity, but the normal host and bootstrap paths do not execute those timed configuration patches (`bd-pcfj.1`). They must not be treated as applied experimental interventions. This is separate from the core drought/expiry intervention records shown by the TUI.

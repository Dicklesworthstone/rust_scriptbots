...
- Set a fixed seed in config: `rng_seed = <u64>`. At runtime you can apply via REST:
  ...
  { "rng_seed": 42 }
  ```
- The root seed deterministically derives all six domain streams and every stable agent/offspring substream. The canonical launch manifest is `scriptbots.run-manifest.v3.3`: it records the six-domain checkpoint, exact `AgentSubstreamProtocolV1`, and UID-ordered `AgentRngCounterStateV1` launch rows. Once the tick-zero start digest is bound, the bootstrap form is `scriptbots.run-manifest.v3.4`; it must bind the same root, protocol, counters, registry semantics, and `WorldDigestV1` evidence rather than describing a different launch state. Neither form collapses the domains back into one ambiguous global stream.
- The bd-1kxd keyed-substream schema and implementation are currently in the code-first/batch-verify phase. Native and WASM acceptance evidence must come from the pinned DSR lane; that final DSR proof is pending and these changes are not yet a closure claim.

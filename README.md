...
- **`scriptbots-runtime`**: Renderer-neutral command, immutable lifecycle evidence, two-axis status, snapshot, and event-cursor contracts with typed revision domains; the sole-owner `HostCore`; a pure injected-time fixed-deadline adapter; and an optional Asupersync `=0.5.0` native lifecycle (migration validation pending). It depends on core but not storage, servers, or renderers. The default and WASM-facing surface remains runtime-neutral; Asupersync is activated only by `native-asupersync` on native targets.
...
- `scriptbots-runtime`: the protocol boundary, typed command/revision/status domains, schema-v1 lifecycle evidence, opaque client ports, cursors, manual-drive contract, sole-owner `HostCore`, pure fixed-deadline driver, and optional current-thread Asupersync lifecycle are implemented. The app uses `HostCore` through its owner thread and channel ports. The `bd-2z0.5.2` lifecycle work and the `bd-pcfj` frontend cutover are closed.
...
- `scriptbots-storage`: the exact FrankenSQLite source is pinned; its bounded worker now has a file-backed durable outbox, stable per-batch identities, monotonic admitted/applied/durable watermarks, ordered startup recovery, controller wait deadlines, supervised worker ownership, exact recovery identity/schema proof, and a V6 run-scoped base schema with V7 canonical host archives. Code-first V8 domain-event and V9 command-lifecycle projections are committed under `bd-2z0.5.2`, with centralized DSR proof pending. Same-thread writes use the same outbox protocol, and terminal receipt/join errors preserve one exact typed root cause. Pairwise combat and food-share edges are projected into indexed SQL with stable UIDs, bounded window sampling, and persisted completeness counters. Deadlines do not cancel an in-flight database call or bound the supervised reaper. Hard-bounded SQL reads are refused before opening the database; frontends use immutable analytics snapshots, while offline/reporting SQL uses the synchronous, create-free, read-only `StorageReader` with explicitly unbounded execution. Strict-run host policy and product checkpoint/replay integration remain open. Run bundles are implemented: `RunBundleV1` (`scriptbots.run-bundle.v1`) with `bundle::create_run_bundle`, `create_run_bundle_from_artifacts`, and `verify_run_bundle`, surfaced as `--create-bundle`/`--bundle-output` and `--verify-bundle`; the two competing implementations were converged under `bd-4d9j`.
...
- `SCRIPTBOTS_BRAIN` — environment-layer brain preset (`mixed|mlp|dwraon|assembly|ft|neuro`, default `mixed`); `--brain` outranks it as a distinct CLI layer. Selecting `ft` without the `brain-ft` build feature fails closed at startup; selecting an unknown string fails clap parsing before any storage is reserved.
...
- Both GPUI windows read immutable publications from the same `HostCore` on the application owner thread. Run controls submit commands to that owner; painting and window refresh rates do not advance scientific time.
...
- **Deliberately different:** combat runs every tick with reach `spike radius + spike length`, multiplicative damage bonuses and partial spike retraction, where the C++ checks every second tick for `d < 2r` within π/8 and snaps the spike to zero; health and energy are separate budgets; reproduction may take a partner (with probability `partner_chance`, the oldest other agent) and yields one child, where the C++ is asexual with `BABIES=2`; the population floor defaults to `population_minimum = 0` instead of topping up to `NUMBOTS=70`; food also regrows continuously in every cell (`food_growth_rate = 0.05`), where the C++ only refills one random cell to `FOODMAX` every 15 ticks.
- **Known default-launch behaviour:** there is no population ceiling. Measured with the default config and founders (seeds 11, 42, 137), population starts from 16 founders and grows past 3,000 agents between ticks ~5,900 and ~7,900. `scenarios/meadow.scenario.toml` passes its 360-tick envelope but passes 3,000 agents by tick ~1,200 on all three declared seeds. Tracked in `bd-vgsx`.
...
- Set a fixed seed in config: `rng_seed = <u64>`. At runtime you can apply via REST:
  ...
  { "rng_seed": 42 }
  ```
- The root seed deterministically derives all six domain streams and every stable agent/offspring substream. The canonical launch manifest is `scriptbots.run-manifest.v3.3`: it records the six-domain checkpoint, exact `AgentSubstreamProtocolV1`, and UID-ordered `AgentRngCounterStateV1` launch rows. Once the tick-zero V1.7 start digest is bound, the bootstrap form is `scriptbots.run-manifest.v3.6`; it must bind the same root, protocol, counters, registry semantics, locomotion model, and `WorldDigestV1` evidence rather than describing a different launch state. Neither form collapses the domains back into one ambiguous global stream.
- The `bd-1kxd` keyed-substream schema and the `bd-2i1` V1.6 locomotion contract are closed with pinned DSR evidence. The reviewed exact-class full performance baseline was promoted byte-for-byte in `39fc2f0`, and the final same-class comparison passed in DSR `0.1.0-bd2i1-perf-compare-quiet1.20260716T160817Z`. `bd-hiv1` is closed: movement-noise sensing and spike-speed damage now consume a typed peak-wheel-effort lane rather than physical displacement (`scriptbots-core` `wheel_effort`; `spike_speed_damage_bonus` scales peak normalized wheel output, and `speed_mag` no longer contributes).
...
- Production CLI/REST/MCP generation and application remain unconnected (`bd-2z0.10.6`); the later interactive rule/sample sandbox and its real-process/native acceptance are tracked by `bd-2z0.10.7` and `bd-2z0.10.8`. Core generation and application APIs do not yet provide that user workflow. The hydrology system builds atop terrain for dynamic water flows (see below).
...
- **One embedded engine:** ScriptBots uses the public `fsqlite` facade from FrankenSQLite with `version = "=0.4.0"`, pinned to immutable revision `a855a15399a1994943c81e15f284bed780b4f86b` at `https://github.com/Dicklesworthstone/frankensqlite`. Defaults remain disabled; `native`, `json`, `fts5`, `icu`, `misc`, and `rtree` preserve the previous native feature's implicit extension surface. Create-free existing-file open and expected-identity verification still precede recovery access. Native validation of this dependency migration is pending.
...
- **V8 scientific evidence:** every durable scientific sequence has an archive-bound projection batch, including honest zero-row boundaries. Exact births, exact deaths, and nonzero aggregate-combat counters retain deterministic local order. Bounded typed evidence/pages reject sequence gaps, duplicate or corrupt ordinals, fabricated cursors, and an expected-but-empty scenario with typed `NoEvidence`. Pairwise combat and food-share events are queryable in the indexed `interactions` table, joined one-to-one to their typed replay source.
- **V9 command evidence:** normalized records retain the exact command envelope, source/client identity, optional admission order, all three revision guards, terminal boundary, and canonical archive digest. Runtime `admitted`/`applied`/`rejected`/`failed` transitions remain distinct from storage `committed_volatile`/`durable` transitions. Finished files expose bounded `StorageReader::command_journal_evidence`, `command_journal_record`, and `command_journal_page`; a nonempty pre-V9 archive is refused before migration because archive v1 cannot supply missing lifecycle evidence.
...
- **Hard-bounded SQL is refused:** the dependency migration does not establish a guaranteed wall-clock stop across engine work or blocking I/O on the `!Send + !Sync` connection-owner worker. `AsyncReadLane::open(path, maximum_duration)` therefore still always returns a typed `ReadExecutionBoundUnavailable` before the path is opened or inspected; there is no async SQL escape hatch. Use `AnalyticsSnapshotProvider::snapshot()` for lock-free, non-SQL frontend latest-state access. Offline/reporting callers that can wait may use the synchronous, create-free, read-only `StorageReader`; its row limits bound results and allocation, not scan, aggregation, sort, open, close, or wall-clock work.

### Tables and query examples

The V6 base schema run-scopes scientific and operational tables for matched-seed experiments. V7 adds the canonical host archive/ledger, V8 adds per-scientific-boundary domain evidence, and V9 adds normalized command records with separate application/storage transitions. Later migrations through the current V19 (`SCRIPTBOTS_SCHEMA_VERSION`) add replay interaction edges, command claims, full-text narrative search rebuilds, and the MAP-Elites archive. Unsupported pre-release data is refused rather than guessed or destructively rewritten. Use the result- and allocation-bounded typed command/domain readers above for journal evidence; those bounds do not promise a wall-clock execution deadline. The SQL below illustrates explicitly unbounded, run-scoped scientific reporting only.
...
- Endpoints:
  ...
  - `POST /api/v1/experiments` → start a matched-seed batch (`variants` × `seeds`, `ticks_per_run`) that executes on a
    background worker; `GET /api/v1/experiments/{id}` reports per-run state, final digest and bundle path;
    `POST .../cancel` stops before the next wave, `POST .../resume` continues pending runs
  - `POST /api/v1/checkpoints` → capture a `WorldCheckpointV1` and record it in the run database (usable by
    `--checkpoint-start`); `GET /api/v1/checkpoints`, `/api/v1/artifacts`
...
- **Replay status**: `--replay-db` and `--compare-db` load, re-simulate, and diff encoded event streams. Production instrumentation projects per-agent actions in canonical `AgentUid` order through `replay_event_tick_cap` (default `512`), budgeted per tick (`bd-a9h1`, fixed). Typed combat and food-share edges preserve source tick, core ordinal, stable actor/target UIDs, positions, and magnitude in the indexed FrankenSQLite `interactions` table. `interaction_event_tick_stride=1` (the default) selects every tick, `N > 1` selects ticks divisible by `N`, and `0` selects aggregates-only; `interaction_event_tick_cap` (default `512`, `0` also aggregates-only) is the hard ceiling on each selected tick. Both knobs are serialized with run provenance. Durable `interaction_events_observed`, `interaction_events_persisted`, `interaction_events_sampled_out`, and `interaction_events_truncated` counters make completeness auditable, while shutdown logs the corresponding run totals at info. The default 5k-agent shape retains at most 512 edges per selected tick and is covered by the bounded-admission/flush gate; raise the cap only after checking the run's payload budget. The mock-free terminal→FrankenSQLite→export→replay E2E is `crates/scriptbots-app/tests/replay_e2e.rs`, and nonzero empty-vs-empty runs fail closed instead of claiming verification.
...
- Tools exposed (30):
  ...
  - Scenarios: `list_presets` → available scenario presets; `apply_preset` → accepts `{ name }`; `intervene` → scheduled intervention
  ...
  - Observation: `get_status` → current simulation status; `get_command_status` → two-axis status by command ID;
    `narrative_search`, `narrative_around` → narrative events; `get_version`, `get_schema`
  - Maps: `map_generate` → procedural sandbox map generation; `map_apply` → apply map artifact to running simulation
  - Experiments: `experiment_create` runs a real matched-seed batch (MLP/DWRAON/Assembly arms × seeds) on a
    background worker and writes a verified run bundle per run; `experiment_status`, `experiment_list`,
    `experiment_cancel` (stops before the next wave), `experiment_resume` (continues pending runs)
  - Checkpoints and artifacts: `checkpoint_create` (captured from the world owner and recorded in the run database
    through the storage worker), `checkpoint_list`, `checkpoint_status`, `artifact_list`, `artifact_get`
...
- **Scheduled patches**: scenario `interventions` run on the owner immediately before the transition out of their declared tick, including tick zero and bootstrap. Entries run in tick order, with document order preserved for the same tick, and merge against the current configuration. Each outcome records its schedule identity, actual boundary revisions, and changed fields separately from journal commitment; later science waits for that commitment. Invalid patches stop science and initiate ordered shutdown. The TUI reports observed outcomes and any gaps in its retained history, separately from core drought/expiry interventions.- **Current state**: repeatable scenarios can layer TOML or RON files with repeated `--config` flags or the semicolon-separated `SCRIPTBOTS_CONFIG` variable; later layers override earlier ones before environment overrides.
  ```text
...
- **Already implemented**
  ...
  - ✅ Production RNG, brain-output, action and world-digest event instrumentation, with the action cap budgeted per tick (`bd-a9h1`).
...
### Owner command architecture
- The live native app transfers its world to `HostThread`. REST/MCP, GPUI, Bevy, TUI, and headless clients use channel ports to that same owner.
- `scriptbots-runtime` provides stable command identity, one external admission order, typed revision guards checked at the application boundary, independent application/journal statuses, immutable snapshots, and independent event cursors.
- `HostCore` applies commands and advances science. Its fixed-deadline driver handles wakeups and bounded catch-up, while ordered shutdown waits for the required journal commitment. Browser ownership is a separate path.
...
4. Storage: durable outbox, exact identity/schema recovery, direct-write/root-cause unification, V6/V7 base/archive lineage, indexed pairwise interaction edges, and run bundles are integrated. V8/V9 command/domain projections are integrated (V8 has no production reader yet); remaining work includes strict-run host policy, automatic checkpoint recording in ordinary runs, and in-memory commit scaling (`bd-kgob`).
5. Runtime: renderer-neutral protocol, sole-owner `HostCore`, fixed-deadline native lifecycle, and frontend ports are integrated; complete remaining client interactions and native acceptance for the host cutover.

...
| `fsqlite` (`=0.4.0`, rev `a855a15399a1994943c81e15f284bed780b4f86b`) | FrankenSQLite persistence for simulation metrics, replay, and run artifacts; migration validation pending |
...
- **One engine:** use the public `fsqlite` facade at package version `0.4.0`, pinned to immutable revision `a855a15399a1994943c81e15f284bed780b4f86b` from `https://github.com/Dicklesworthstone/frankensqlite`. The workspace declaration uses `version = "=0.4.0"`, `default-features = false`, and `features = ["native", "json", "fts5", "icu", "misc", "rtree"]`. These extensions preserve the previous native feature's implicit SQL surface; the Asupersync 0.5 migration still requires native and centralized DSR validation.

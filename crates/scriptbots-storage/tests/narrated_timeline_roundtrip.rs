...
#[test]
#[ignore = "manual read-only diagnostic requires SCRIPTBOTS_READBACK_DB"]
fn retained_database_readback_diagnostic() {
    let path = std::env::var("SCRIPTBOTS_READBACK_DB").expect("retained database path");
    eprintln!("readback: opening");
    let reader = StorageReader::open(&path).expect("retained reader opens");
    eprintln!("readback: querying");
    let rows = reader.recent_metrics(301).expect("retained metrics read");
    assert_eq!(rows.len(), 300);
    eprintln!("readback: closing");
    reader.close().expect("retained reader closes");
    eprintln!("readback: verified 300 rows");
}

...
fn a_food_draw_moves_only_the_food_rng_domain() {
    ...
    let before = control.world_digest_v1().expect("control V1.6 digest");
    let _ = food_advanced
        .rng(RngDomain::Food)
    ...
    let after = food_advanced
        .world_digest_v1()
        .expect("food-advanced V1.6 digest");
...
fn v1_6_wire_is_exact_and_rejects_older_contracts() {
    ...
    let digest = world_with_brain_seed(17, 2)
        .world_digest_v1()
        .expect("V1.6 wire fixture");
    let json = serde_json::to_value(&digest).expect("encode V1.6 JSON");
    let json_round_trip: WorldDigestV1 =
        serde_json::from_value(json.clone()).expect("decode V1.6 JSON");
    ...
    let rng = json["rng"].as_object().expect("V1.6 RNG digest object");
    ...
    assert_eq!(
        ...
        "V1.6 RNG diagnostics must carry exactly one aggregate and one domain map"
    );
    let domain_keys: BTreeSet<String> = rng
        .get("domains")
        .and_then(serde_json::Value::as_object)
        .expect("V1.6 RNG domain map")
        .keys()
        .cloned()
        .collect();
    ...
    missing_domain["rng"]["domains"]
        .as_object_mut()
        .expect("mutable V1.6 RNG domains")
    ...
    assert!(
        ...
        "an adapter-blind V1.3 contract satisfied the V1.6 identity boundary"
    );
    ...
    assert!(
        ...
        "single-stream JSON V1.2 payload decoded as V1.6"
    );
    ...
    if let Ok(decoded) = postcard::from_bytes::<WorldDigestV1>(&legacy) {
        assert!(
            ...
            "single-stream positional V1.2 payload satisfied the V1.6 contract"
        );
    }
}
